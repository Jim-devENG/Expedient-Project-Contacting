...
var objectFitVideos=function(t){"use strict";function e(t){for(var e=getComputedStyle(t).fontFamily,o=null,i={};null!==(o=l.exec(e));)i[o[1]]=o[2];return i["object-position"]?n(i):i}
...
function r(t,e,o){o=o||window;var i=!1,n=null;try{n=new CustomEvent(e)}catch(t){n=document.createEvent("Event"),n.initEvent(e,!0,!0)}
var r=function(){i||(i=!0,requestAnimationFrame(function(){o.dispatchEvent(n),i=!1}))};o.addEventListener(t,r)}
var d=navigator.userAgent.indexOf("Edge/index.html")>=0,a=new Image,c="object-fit"in a.style&&!d,p="object-position"in a.style&&!d,l=/(object-fit|object-position)\s*:\s*([-\w\s%]+)/g;c&&p||(o(t),r("resize","optimizedResize"))};"undefined"!=typeof module&&"undefined"!=typeof module.exports&&(module.exports=objectFitVideos);
...
String.prototype.replaceAll=function(str1,str2,ignore){str1=str1.toString();str2=str2.toString();if(!str1||!str2){return false;}
...
(function($){$('.wpcf7 label').each(function(i,v){});$('.wpcf7 select').each(function(){$(this).parent('span').addClass('common-select');})
function assign_new_id(el,old_id){let new_id=old_id+'-'+(1+Math.floor(Math.random()*1000));if($('#'+new_id).length>0){assign_new_id(el,old_id)}else{$('#'+old_id).attr('id',new_id);el.attr('for',new_id);}}})(jQuery);
...
(function($){var main_popup=$('.main-popup');var hide_popup=sessionStorage.getItem("hide_main_popup");if(hide_popup=="y"){main_popup.remove();}else{main_popup.addClass('active');}
$('.main-popup').on('click',function(){sessionStorage.setItem("hide_main_popup","y");main_popup.remove();});})(jQuery);
(function($){let mega_menu=$('.nav-desktop .is-mega.type-b');if(mega_menu.length==0){return false;}
mega_menu.each(function(){let curr_mega=$(this);let current_page_el=curr_mega.find('.current-menu-item');curr_mega.find('.sub-menu').removeClass('sub-menu').addClass('sub-menu-list');curr_mega.find('.sub-menu-list').wrap('<div class="sub-menu"></div');let mega_wrap=curr_mega.find('.sub-menu');mega_wrap.append('<div class="imgs-wrap"></div>');let imgs_wrap=mega_wrap.find('.imgs-wrap');let menu_icons=curr_mega.find('.menu-icon');menu_icons.each(function(){imgs_wrap.append($(this));});let default_img=current_page_el.length>0?menu_icons.eq(current_page_el.index()+1):menu_icons.first();let active_img=default_img;menu_icons_toggle(menu_icons,active_img);curr_mega.find('.menu-item').on('mouseenter',function(){let i=$(this).index();let active_img=menu_icons.eq(i+1);menu_icons_toggle(menu_icons,active_img);});mega_wrap.on('mouseleave',function(){let active_img=default_img;menu_icons_toggle(menu_icons,active_img);});});function menu_icons_toggle(menu_icons,active_img){menu_icons.removeClass('active');active_img.addClass('active');}})(jQuery);
(function($){$('.js-modal-init').attr({'aria-expanded':'false','aria-haspopup':'true',});$(document).on('keydown',function(event){if(event.keyCode==27){close_modals(true);}});$('.js-modal-init').on('click',function(e){var that=$(this);var popup=that.find(".js-modal");popup=popup.length?popup:that.next('.js-modal');var is_open=popup.hasClass('open');$('.js-modal-init.open').not(that).removeClass('open');$('.js-modal.open').not(popup).removeClass('open');if(!is_open){that.addClass('open');popup.addClass('open');that.attr('aria-expanded','true');}else{that.removeClass('open');popup.removeClass('open');that.attr('aria-expanded','false');}
e.preventDefault();e.stopPropagation();});$('.js-modal-close').on('click',function(){close_modals(true);});$('.js-modal *').not('.js-modal-close').on('focusout',function(){setTimeout(function(){var is_in_modal=$(document.activeElement).closest('.js-modal').length;if(!is_in_modal){close_modals();}},10);});$('.js-modal').on('click',function(e){var target=e.target;var el_type=target.nodeName;var is_close_btn=$(target).hasClass('js-modal-close');if(el_type!="A"&&!is_close_btn){e.stopPropagation();}});$('body').on('click',function(){close_modals();});function close_modals(focus=false){var open_modal_init=$('.js-modal-init.open');$('.js-modal.open').removeClass('open');if(focus){open_modal_init.focus();}
open_modal_init.removeClass("open").attr('aria-expanded','false');}})(jQuery);
...
(function($){if($('.page-template-template-careers').length==0){return false;}
...
function get_chosen_filters(){let chosen_filters=[];$('.filter').each(function(){let el=$(this);let object={filter_type:null,filter_values:[],};let _filter_type=$(el.find('.filter-input')[0]).data('filter-type');object.filter_type=_filter_type;el.find('.filter-input').each(function(){let el=$(this);let value=el.val();if($(this).attr('type')=="checkbox"){value=el.is(":checked")?el.val():false;}
if(value){value=parseInt(value);object.filter_values.push(value);}});if(object.filter_values.length>0){chosen_filters.push(object);}});return chosen_filters;}
...
function show_no_results_msg(){$('.no-results-msg').addClass('active');}})(jQuery);
function toggle_videos(slider,currentSlide){var all_videos=slider.find("video");var active_video=slider.find('[data-slick-index='+currentSlide+']').find("video");if(all_videos.length>0){all_videos[0].pause();all_videos[0].currentTime=0;}
if(active_video.length>0){active_video[0].play();}};
(function($){let sliders=$('.common-slider .slider');if(sliders.length==0){return false;}
...
(function($){if($('.table').length==0){return false;}
...
function remove_togglers(){$('.footable-toggle').remove();}})(jQuery);
...
(function($){let search_terms=$('.hidden-search-terms .js-hidden-search-term');if(search_terms.length==0){return false;}
...
(function($){let search_form=$('.hero .search-form');if(search_form.length==0){return false;}
let search_input=search_form.find('.js-search-form-input');let search_btn=search_form.find('.js-submit-btn');let results=search_form.find('.results');search_form.on('click',function(event){var mq=window.matchMedia("(max-width: 450px)");if(mq.matches){setTimeout(function(){$('html, body').animate({scrollTop:search_form.offset().top-25});},600);}
toggle_display_items(true);event.stopPropagation();});$(window).on('click',function(){toggle_display_items(false);});function toggle_display_items(is_show){if(is_show){search_input.addClass('active');search_btn.addClass('active');results.addClass('active');return;}
search_input.removeClass('active');search_btn.removeClass('active');results.removeClass('active');}})(jQuery);
(function($){let search_form=$('.hero .search-form');if(search_form.length==0){return false;}
...
(function($){let search_form=$('.hero .search-form');if(search_form.length==0){return false;}
$(document).on('hero-search-submit',function(){let term=$('.js-search-form-input').val();wp.ajax.post("jwd_add_search_terms",{search_term:term,search_term_nonce:jwd_nonce_vars.jwd_search_term_nonce}).done(function(response){});});$('.hero .search-form').on('keypress',function(e){if(e.which==13){$('.hero .js-submit-btn').click();}});})(jQuery);
(function($){let services=$('.home .services');if(services.length==0){return false;}
services.find('.nav-list .item-btn').on('click',function(){let el=$(this);let toggles=el.data('toggles');services.find('.active').removeClass('active');el.find('.img,.title').addClass('active');$(toggles).addClass('active');$('html, body').animate({scrollTop:$(toggles).offset().top-65},function(){$(toggles).focus();});});})(jQuery);
...
(function($){var images=window.location.origin+"/wp-content/themes/jwd-master/images";sideSliderMenu(true,true);function sideSliderMenu(logo,fadeBanner){$('#js-mobile-menu-toggle').on('click',function(){let menu=$(this).attr('data-toggles');slide_menu_animation(menu,fadeBanner);});$('#mobile-menu .sub-menu').each(function(){$(this).prepend('<li class="sub-menu-header">'+'<button class="sub-menu-back-button">'+'<img class="arrow left-arrow style-svg" src="'+images+'/pull-menu-left.svg" alt="Toggle Previous menu" />'+'</button>'+
$(this).prev('a').text()+'</li>');});$('#mobile-menu .menu-item-has-children').each(function(){$('> a',this).wrap('<div class="menu-item-has-children-items-wrapper">').after('<img class="arrow right-arrow style-svg" src="'+images+'/pull-menu-right.svg" alt="Toggle sub menu" />')});$('#mobile-site-navigation').prepend('<button class="close-btn" data-toggles="#mobile-menu">'+'X'+'<span class="screen-reader-text">Click to toggle menu</span>'+'</button>');$('#mobile-site-navigation .close-btn').on('click',function(){let menu=$(this).attr('data-toggles');slide_menu_animation(menu,fadeBanner,false,false);});$('#mobile-menu .menu-item-has-children').on('click',function(){let menu=$('> .sub-menu',this);let is_slide_in=!menu.hasClass('active');if(is_slide_in){slide_menu_animation(menu,false);}else{slide_menu_animation(menu,false,false);}
return false;});$('#mobile-menu .menu-item-has-children .sub-menu .menu-item:not(.menu-item-has-children) a').on('click',function(e){e.stopPropagation();});if(logo){$('#mobile-menu').prepend('<li class="menu-logo-wrapper">'+'<a href="'+localizedVar.homeURL+'" >'+
...
function animate_close_btn(anim_in){let close_btn=$('#mobile-site-navigation').find('.close-btn');if(anim_in){close_btn.addClass('active');setTimeout(function(){close_btn.addClass('active-b')},300);}else{close_btn.removeClass('active active-b');}}})(jQuery);
...
strDate+=':';}}
var date=new Date(str_replace(strDate,'-','../index.html')),res=0;if(date){res=date.getTime();}
return res;}
